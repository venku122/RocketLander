...
const perlinSize = 10000;
const xMax = map_range(Math.random(), 0, 1, 0, 100);
const yMax = map_range(Math.random(), 0, 1, 0, 100);
...
// Function to linearly interpolate between a0 and a1
 ...
 function dotGridGradient(ix, iy, x, y) {
	 ...
	 const nodeVector = gradientNode(ix, iy);
...
function perlin(x, y) {
	...
	const x0 = (x > 0.0 ? x : x - 1);
	const x1 = x0 + 1;
	const y0 = (y > 0.0 ? y : y - 1);
	const y1 = y0 + 1;
